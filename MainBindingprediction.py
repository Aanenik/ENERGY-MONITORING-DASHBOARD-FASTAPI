...
@app.get("/meter_dashboard", response_class=HTMLResponse)
def meter_dashboard(mtid: str):
    ...
    try:
        ...
        if df.empty:
            return HTMLResponse("<h3>No data found for this MTID today</h3>")
...
<p>Average PF : {avg_pf}</p>
<p>Maximum PF : {max_pf}</p>
<p>Minimum PF : {min_pf}</p>
<p>Status : {pf_status}</p>

<h2>Energy</h2>

<p>Total Energy Today : {total_kwh} kWh</p>
<p>Maximum Demand : {maximum_demand_kva} kVA</p>

<h2>TOD Zone Energy</h2>

<p>A Zone : {zones['Zone A']} kWh ({zone_percent['Zone A']}%)</p>
<p>B Zone : {zones['Zone B']} kWh ({zone_percent['Zone B']}%)</p>
<p>C Zone : {zones['Zone C']} kWh ({zone_percent['Zone C']}%)</p>
<p>D Zone : {zones['Zone D']} kWh ({zone_percent['Zone D']}%)</p>
